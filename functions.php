...
function obtener_post_por_id($conexion, $IdPintor){
	$resultado = $conexion->query("SELECT * FROM pintores WHERE IdPintor = $IdPintor LIMIT 1");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}

#funcion para obtener los cuadros de los pintores
function obtener_cuadros($conexion, $IdPintor){
	$resultado = $conexion->query("SELECT cuadros.Codigo, cuadros.Titulo, cuadros.Dimensiones, cuadros.Fecha,cuadros.Tecnica, cuadros.thumb, cuadros.IdPin, cuadros.IdPintor
									FROM cuadros 
									INNER JOIN pintores ON cuadros.IdPintor = pintores.IdPintor
									WHERE pintores.IdPintor = $IdPintor");
	$resultado = $resultado->fetchAll();
	return ($resultado) ? $resultado : false;
}


function id_articulo($IdPintor){
	return (int)limpiarDatos($IdPintor);
}
...
function obtener_post($post_por_pagina, $conexion){
	...
	$sentencia = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM pintores LIMIT {$inicio}, {$post_por_pagina}");
